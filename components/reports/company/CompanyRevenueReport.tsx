...
export function CompanyRevenueReport({ startDate, endDate }: CompanyRevenueReportProps) {
    ...
    const fetchData = async () => {
      ...
      try {
        const [statsData, trendsData] = await Promise.all([
          fetchCompanyRevenueStatsAction(startDate.toISOString(), endDate.toISOString()),
  ...
  const kpis = stats ? [
    ...
    {
      title: 'Crecimiento',
      value: `${stats.growth_percentage >= 0 ? '+' : ''}${stats.growth_percentage.toFixed(1)}%`,
    ...
    },
    {
      ...
      subtitle: `Prom: ${formatCurrency(stats.avg_revenue_per_business / 100)}/negocio`,
      icon: Building2
    },
    {
      title: 'Mejor Negocio',
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {loading ? (
          Array.from({ length: 4 }).map((_, i) => (
            <KPICard key={i} title="" value="" subtitle="" icon={DollarSign} loading />
          ))
        ) : (
          kpis.map((kpi, i) => (
            <KPICard
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm text-muted-foreground mb-4">
                ...
                <span>
                  Total: {formatCurrency(trends.reduce((sum, t) => sum + t.revenue, 0) / 100)}
                </span>
              </div>
                ...
                {trends.map((trend, i) => {
                  ...
                  return (
                    <div
                      ...
                      title={`${trend.date}: ${formatCurrency(trend.revenue / 100)}`}
                    />
                  )
                })}
              </div>
