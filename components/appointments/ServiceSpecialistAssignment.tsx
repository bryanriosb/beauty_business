...
import { Check, Users, AlertCircle, ChevronDown, ChevronUp, Clock, Split, Merge } from 'lucide-react'
...
export default function ServiceSpecialistAssignmentComponent({
  ...
  const [categoryGroups, setCategoryGroups] = useState<CategorySpecialistsMap[]>([])
  const [universalSpecialists, setUniversalSpecialists] = useState<SpecialistWithCategories[]>([])
  const [canSingleSpecialistHandleAll, setCanSingleSpecialistHandleAll] = useState(false)
  const [useUnifiedAssignment, setUseUnifiedAssignment] = useState(true)
  const [expandedCategories, setExpandedCategories] = useState<Set<string>>(new Set())

  // State for each category's assignment
  const [categoryAssignments, setCategoryAssignments] = useState<Map<string, CategoryAssignmentState>>(new Map())

  // Unified mode state
  const [unifiedSpecialistId, setUnifiedSpecialistId] = useState<string | null>(null)
      ...
      try {
        ...
        if (result.success) {
          ...
          if (!initialLoadDoneRef.current) {
            if (!result.canSingleSpecialistHandleAll || result.universalSpecialists.length === 0) {
              setUseUnifiedAssignment(false)
              const allCategoryKeys = new Set(result.categoryGroups.map(g => g.categoryId || 'no-category'))
  ...
  useEffect(() => {
    async function loadUnifiedSlots() {
      if (!useUnifiedAssignment || !unifiedSpecialistId || !businessId || !date || services.length === 0) {
        setUnifiedSlots([])
        return
      }
      ...
      try {
        ...
        if (result.success && result.slots) {
          ...
          const availableSlots = result.slots.filter(
            slot => slot.available && slot.availableSpecialistIds.includes(unifiedSpecialistId)
          )
    ...
    loadUnifiedSlots()
  }, [useUnifiedAssignment, unifiedSpecialistId, businessId, date, services, excludeAppointmentId])

  // Handle unified specialist selection
  const handleUnifiedSpecialistSelect = useCallback((specialist: SpecialistWithCategories) => {
    if (disabled) return
  ...
  const handleUnifiedTimeSelect = useCallback((time: string) => {
    if (disabled || !unifiedSpecialistId) return
    setUnifiedStartTime(time)

    const specialist = universalSpecialists.find(s => s.id === unifiedSpecialistId)
    ...
    const newAssignments = value.map(assignment => {
      ...
      const serviceStartTime = `${Math.floor(currentMinutes / 60).toString().padStart(2, '0')}:${(currentMinutes % 60).toString().padStart(2, '0')}`
      const serviceEndTime = `${Math.floor(serviceEndMinutes / 60).toString().padStart(2, '0')}:${(serviceEndMinutes % 60).toString().padStart(2, '0')}`
  ...
  }, [disabled, unifiedSpecialistId, universalSpecialists, totalDuration, value, onChange, onTimesCalculated])

  // Load slots for a specific category/specialist
  const loadCategorySlotsForSpecialist = useCallback(async (
    categoryKey: string,
    specialistId: string,
    ...
    setCategoryAssignments(prev => {
      const newMap = new Map(prev)
      const current = newMap.get(categoryKey) || { specialistId: null, startTime: null, availableSlots: [], isLoadingSlots: false }
      newMap.set(categoryKey, { ...current, specialistId, isLoadingSlots: true, startTime: null })
      return newMap
    })
  ...
  const handleCategorySpecialistSelect = useCallback((
    ...
    if (disabled) return
    loadCategorySlotsForSpecialist(categoryKey, specialist.id, categoryDuration, serviceId)

    if (!onPrimarySpecialistChange) return
    const hasAnySpecialist = Array.from(categoryAssignments.values()).some(a => a.specialistId)
    if (!hasAnySpecialist) {
      onPrimarySpecialistChange(specialist.id)
    }
  }, [disabled, loadCategorySlotsForSpecialist, categoryAssignments, onPrimarySpecialistChange])
  ...
  const handleCategoryTimeSelect = useCallback((
    ...
    const newAssignments = value.map(a => {
      if (a.categoryId === categoryId || (categoryId === null && a.categoryId === null)) {
    ...
    const allStartTimes = newAssignments.filter(a => a.startTime).map(a => a.startTime!)
    const allEndTimes = newAssignments.filter(a => a.endTime).map(a => a.endTime!)
  ...
  }, [disabled, categoryAssignments, categoryGroups, value, onChange, onTimesCalculated])
  ...
  const allBlockedTimeRanges = useMemo(() => {
    const ranges: Array<{ start: number; end: number; categoryId: string | null }> = []
  ...
  const isSlotBlocked = useCallback((
    ...
    for (const range of allBlockedTimeRanges) {
      ...
      if (range.categoryId === currentCategoryId ||
  ...
  return (
    <div className="space-y-4">
      ...
      {canSingleSpecialistHandleAll && universalSpecialists.length > 0 && useUnifiedAssignment && (
        <div className="space-y-3">
          ...
          <p className="text-xs text-muted-foreground">
            {universalSpecialists.length} especialista{universalSpecialists.length !== 1 ? 's' : ''} puede{universalSpecialists.length !== 1 ? 'n' : ''} atender todos los servicios
          </p>
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    !isSelected && 'bg-card hover:bg-accent/50 border-border hover:border-primary/30',
            ...
            <div className="space-y-2 pt-2 border-t">
              ...
              {isLoadingUnifiedSlots ? (
                ...
                <p className="text-xs text-amber-600 dark:text-amber-400">
                  No hay horarios disponibles para este especialista en esta fecha
                </p>
              ) : (
                <div className="flex gap-2 flex-wrap">
      ...
      {(!useUnifiedAssignment || !canSingleSpecialistHandleAll || universalSpecialists.length === 0) && (
        <div className="space-y-3">
          ...
          {(!canSingleSpecialistHandleAll || universalSpecialists.length === 0) && (
            <div className="flex items-center gap-2 p-3 rounded-lg bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800">
              ...
              <p className="text-xs text-amber-700 dark:text-amber-400">
                Los servicios seleccionados requieren diferentes especialistas. Asigna especialista y horario para cada categoría.
              </p>
            </div>
          )}
          ...
          {categoryGroups.map((group) => {
            ...
            const servicesInGroup = value.filter(v =>
              v.categoryId === group.categoryId || (group.categoryId === null && v.categoryId === null)
            )

            const categoryDuration = servicesInGroup.reduce((sum, s) => sum + s.durationMinutes, 0)
            const groupAssigned = servicesInGroup.every(v => v.specialistId !== null && v.startTime !== null)
                  ...
                  <CollapsibleTrigger asChild>
                    <button
                      ...
                      className={cn(
                        ...
                        groupAssigned ? 'bg-green-50 dark:bg-green-950/20' : 'bg-card'
                      )}
                    >
                      <div className="flex items-center gap-2 flex-wrap">
                        <Badge variant={groupAssigned ? 'default' : 'secondary'} className="text-xs">
                          {group.categoryName || 'Sin categoría'}
                        </Badge>
                        <span className="text-xs text-muted-foreground">
                          {servicesInGroup.length} servicio{servicesInGroup.length !== 1 ? 's' : ''} · {categoryDuration} min
                        </span>
                        {groupAssigned && (
                  ...
                  <CollapsibleContent>
                    <div className="p-3 border-t space-y-3">
                      <div className="text-xs text-muted-foreground">
                        Servicios: {servicesInGroup.map(s => s.serviceName).join(', ')}
                      </div>

                      {/* Specialist Selection */}
                      <div className="space-y-2">
                        <p className="text-xs font-medium">Selecciona especialista:</p>
                        {group.specialists.length === 0 ? (
                          <p className="text-xs text-amber-600">No hay especialistas disponibles para esta categoría</p>
                        ) : (
                          <div className="grid grid-cols-2 gap-2">
                            {group.specialists.map((specialist) => {
                              const isSelected = categoryState?.specialistId === specialist.id
                      ...
                      {categoryState?.specialistId && (
                        <div className="space-y-2 pt-2 border-t">
                          ...
                          {categoryState.isLoadingSlots ? (
                            ...
                            const availableFilteredSlots = categoryState.availableSlots.filter(
                              slot => !isSlotBlocked(slot.time, categoryDuration, group.categoryId)
                            )
                            return availableFilteredSlots.length === 0 ? (
                              <p className="text-xs text-amber-600 dark:text-amber-400">
                                No hay horarios disponibles (el cliente tiene otros servicios en esos horarios)
                              </p>
                            ) : (
                              <div className="flex gap-2 flex-wrap">
                                {availableFilteredSlots.map((slot) => (
                                  <button
                                    ...
                                    onClick={(e) => {
                                      e.stopPropagation()
                                      handleCategoryTimeSelect(categoryKey, group.categoryId, slot.time)
                                    }}
                                    className={cn(
        ...
        <div className="p-3 rounded-lg bg-muted/50 space-y-2">
          ...
          <div className="space-y-1">
            {value
              ...
              .map((assignment) => (
                <div key={assignment.serviceId} className="flex items-center justify-between text-xs">
                  <span className="text-muted-foreground truncate flex-1">
                    {assignment.serviceName}
                  </span>
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="font-medium">{assignment.specialistName}</span>
                    <Badge variant="outline" className="text-[10px] px-1.5">
                      {assignment.startTime && formatTime(assignment.startTime)} - {assignment.endTime && formatTime(assignment.endTime)}
                    </Badge>
                  </div>
                </div>
              ))}
