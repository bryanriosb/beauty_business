...
export function SimpleExportButton({
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const finalDateRange = customDateRange.from && customDateRange.to ? {
  ...
  const getActiveFiltersCount = () => {
    return Object.values(exportFilters).reduce((count, values) => count + values.length, 0)
  }
  ...
  const getColumnName = (column: { id: string; header?: string | React.ReactNode }) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>Exportar Datos - {tableName}</DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="excel">Excel (Archivo .xls)</SelectItem>
              </SelectContent>
            </Select>
          </div>
              ...
              <div className="space-y-4 border rounded-md p-3 max-h-48 overflow-y-auto">
                {filterConfigs.map((filter) => (
                  <div key={filter.column} className="space-y-2">
                    <p className="text-sm font-medium text-muted-foreground">{filter.title}</p>
                    <div className="flex flex-wrap gap-2">
                      {filter.options.map((option) => {
                        const isChecked = exportFilters[filter.column]?.includes(option.value) || false
                        return (
                          <div
                            key={option.value}
                            className={cn(
                            ...
                            onClick={() => !isExporting && handleFilterChange(filter.column, option.value, !isChecked)}
                          >
          ...
          <div className="space-y-3">
            ...
            {!exportAll && (
              <div className="space-y-2 max-h-40 overflow-y-auto border rounded-md p-3">
                <p className="text-sm text-muted-foreground mb-2">Selecciona las columnas:</p>
                {columns.map((column) => (
          ...
          {(getActiveFiltersCount() > 0 || searchQuery || customDateRange.from || customDateRange.to) && (
            <div className="space-y-2">
              <Label className="font-medium">Filtros que se aplicarán:</Label>
              <div className="text-sm text-muted-foreground space-y-1">
                {searchQuery && <p>• Búsqueda: &quot;{searchQuery}&quot;</p>}
                {(customDateRange.from && customDateRange.to) && (
                  <p>• Rango fechas: {format(customDateRange.from, "dd/MM/yy", { locale: es })} - {format(customDateRange.to, "dd/MM/yy", { locale: es })}</p>
                )}
                {filterConfigs && Object.entries(exportFilters).map(([column, values]) => {
                  if (values.length === 0) return null
                  const filterConfig = filterConfigs.find(f => f.column === column)
                  if (!filterConfig) return null
                  const labels = values.map(v => filterConfig.options.find(o => o.value === v)?.label || v)
                  return (
                    <p key={column}>• {filterConfig.title}: {labels.join(', ')}</p>
                  )
                })}
              </div>
            </div>
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={isExporting || (!exportAll && selectedColumns.length === 0)}
          >
            {isExporting ? (
              <>
