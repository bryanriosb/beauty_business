...
export function SpecialistDetailPanel({
  ...
  const initials = specialist
    ? `${specialist.first_name[0]}${specialist.last_name?.[0] || ''}`.toUpperCase()
    : ''
  ...
  const availabilityByDay = specialist?.availability?.reduce(
    (acc, avail) => {
          ...
          <div className="p-6 space-y-6">
              ...
              {todayAppointments.length === 0 ? (
                ...
                <div className="space-y-3">
                  {todayAppointments.map((apt, index) => {
                    ...
                    const startTime = new Date(apt.start_time).toLocaleTimeString('es-CO', {
                      hour: '2-digit',
                      minute: '2-digit',
                      hour12: true,
                    })
                    const endTime = new Date(apt.end_time).toLocaleTimeString('es-CO', {
                      hour: '2-digit',
                      minute: '2-digit',
                      hour12: true,
                    })
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className="flex items-center gap-2 text-sm text-muted-foreground">
                            ...
                            <span className={isNow ? 'font-medium text-foreground' : ''}>
                              {isNow && '(Ahora) '}
                              {startTime} - {endTime}
                            </span>
                          </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-xs text-primary font-medium">{formatDateRange()}</span>
              </div>
              ...
              <div className="space-y-2">
                {DAYS_ORDER.map((day) => {
                  ...
                  return (
                    ...
                    >
                      <span className={`w-12 ${isToday ? 'font-medium' : 'text-muted-foreground'}`}>
                        {DAYS_MAP[day]}
                      </span>
                      {schedule ? (
