...
import { ConnectionStatus, EventBridgeEvent } from '@/types/eventbridge'
...
export default function EventBridgeMonitor({
  ...
  setEvents: (events: EventBridgeEvent[]) => void
}) {
  const {
    events,
    isConnected,
  ...
  const resetEvents = (portalName: string) => {
    switch (portalName) {
      case 'client':
        set([])
        setProcessing(false)
  ...
  return (
    <div className="grid gap-2">
      <div className="grid gap-2 max-h-[10rem] w-full overflow-y-auto">
            ...
            >
              <div className="grid gap-2 items-center">
                <span className="text-center bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded">
                  ...
                  {resetEvents(event.detail.portalName)}
                </span>
